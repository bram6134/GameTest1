...
function setup() {
  ...
  forBackground = createSprite(0, 0, width, height);
  forBackground.addImage(bg);
  forBackground.scale = 1.15;
  forBackground.x = forBackground.width/2;
  forBackground.velocityX = -5;

  mario = createSprite(width/10, height-150, 10, 10);
  ...
  ground = createSprite(width/2, height-90, 1000, 25);
  ...
  restart = createSprite(width/2, height-300, 50, 50);
...
function draw() {
  if(gameState===1){
    ...
    if(touches.length > 0  && mario.y >= height-250 || keyDown("space") && mario.y >= height-250){
      mario.velocityY =- 25
      touches = [ ];
    }
...
function Coins(){
  if(frameCount%225===0){
    var coin = createSprite(width,height/2-100,50,50);
...
function Enemy(){
  if(frameCount%125===0){
    var enemy = createSprite(width,height-130,50,50);
